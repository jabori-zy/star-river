...
export async function getStrategyByIdApi(
	...
	// Step 1: Log request (development environment)
	if (import.meta.env.DEV) {
		console.log("[get strategy by id] strategyId:", strategyId);
	}
	...
	try {
		...
		if (import.meta.env.DEV) {
			console.log("[get strategy by id] response data:", data);
		}
...
export function useGetStrategyById(
	...
	return useQuery({
		...
		queryFn: () => {
			...
			if (import.meta.env.DEV) {
				console.log("[get strategy by id] fetching strategy:", strategyId);
			}
