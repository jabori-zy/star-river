...
// 根据交易所，获取账户配置数据
export async function getAccountConfigs(exchange: string | null): Promise<MT5Account[]> {
    // 根据交易所，映射账户配置数据
    if (exchange === "metatrader5") {
      try {
        const response = await axios.get(`${API_URL}/config?exchange=${exchange}`)
        const accountConfigs = response.data.data || []
        const mt5Accounts: MT5Account[] = []
        accountConfigs.forEach((item: {
          id: number
          account_name: string
          exchange: string
          config: {
...
export async function startMt5Terminal(accountId: number) {
  ...
  try {
    const {data} = await axios.post(`${API_URL}/start_mt5_terminal`, requestBody)
    return data
  } catch (error) {
...
// 添加账户配置
export async function addAccountConfig(account_name: string, exchange: string, accountConfig: any) {
  try {
    const requestBody = {
