...
/**
 * Stop backtest strategy request parameters
 */
export interface StopBacktestStrategyRequest {
	strategyId: number;
}

/**
 * Stop backtest strategy response
 */
export interface StopBacktestStrategyResponse {
	success: boolean;
}
...
/**
 * Stop backtest strategy API call (without UI logic)
 *
 * @param params Stop parameters
 * @returns Stop result
 */
export async function stopBacktestStrategyApi(
	params: StopBacktestStrategyRequest,
): Promise<StopBacktestStrategyResponse> {
	...
	if (import.meta.env.DEV) {
		console.log("[stop backtest strategy] strategyId:", params.strategyId);
	}

	try {
		// Step 3: Send POST request
		const response = await axios.post<ApiResponse<StopBacktestStrategyResponse>>(
		...
		if (import.meta.env.DEV) {
			console.log("[stop backtest strategy] success");
		}
 ...
 */
export interface UseStopBacktestStrategyOptions {
	/**
...
/**
 * React Query Hook for stopping backtest strategy
 */
export function useStopBacktestStrategy(options?: UseStopBacktestStrategyOptions) {

	const { meta, onSuccess, onError, onSettled } = options || {};

	return useMutation({
		// Mutation function
		mutationFn: (params: StopBacktestStrategyRequest) => stopBacktestStrategyApi(params),
		...
		onSuccess: () => {
			...
			if (import.meta.env.DEV) {
				console.log("[stop backtest strategy] mutation success");
			}
		},
		...
		onError: (error: Error) => {
			...
			if (import.meta.env.DEV) {
				console.error("[stop backtest strategy] mutation error:", error);
			}
		},
