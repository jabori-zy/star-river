...
export const useUpdateLiveConfig = ({ id, initialConfig }: UseUpdateLiveConfigProps) => {
  ...
  // 统一的状态管理
  const [config, setConfig] = useState<IfElseNodeLiveConfig | undefined>(initialConfig);

  // 通用的更新函数
  const updateConfig = useCallback((updater: (prev: IfElseNodeLiveConfig | undefined) => IfElseNodeLiveConfig) => {
  ...
  // 默认配置值
  const getDefaultConfig = useCallback((prev?: IfElseNodeLiveConfig): IfElseNodeLiveConfig => ({
    cases: prev?.cases || [{caseId: 1, logicalSymbol: LogicalSymbol.AND, conditions: []}],
    ...prev
  }), []);

  // 通用的字段更新方法
  const updateField = useCallback(<K extends keyof IfElseNodeLiveConfig>(
    field: K, 
