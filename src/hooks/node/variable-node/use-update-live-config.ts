...
export const useUpdateLiveConfig = ({ id, initialConfig }: UseUpdateLiveConfigProps) => {
  ...
  // 统一的状态管理
  const [config, setConfig] = useState<VariableNodeLiveConfig | undefined>(initialConfig);
  ...
  // 通用的更新函数
  const updateConfig = useCallback((updater: (prev: VariableNodeLiveConfig | undefined) => VariableNodeLiveConfig) => {
    // 获取最新的节点数据，而不是依赖可能过时的state
    const currentNode = getNode(id);
    const currentConfig = currentNode?.data?.liveConfig as VariableNodeLiveConfig | undefined;
  ...
  // 默认配置值
  const getDefaultConfig = useCallback((prev?: VariableNodeLiveConfig): VariableNodeLiveConfig => ({
  ...
  // 通用的字段更新方法
  const updateField = useCallback(<K extends keyof VariableNodeLiveConfig>(
    field: K, 
