...
export const useUpdateSimulateConfig = ({ id, initialConfig }: UseUpdateSimulateConfigProps) => {
  ...
  // 统一的状态管理
  const [config, setConfig] = useState<VariableNodeSimulateConfig | undefined>(initialConfig);
  ...
  // 通用的更新函数
  const updateConfig = useCallback((updater: (prev: VariableNodeSimulateConfig | undefined) => VariableNodeSimulateConfig) => {
    // 获取最新的节点数据，而不是依赖可能过时的state
    const currentNode = getNode(id);
    const currentConfig = currentNode?.data?.simulateConfig as VariableNodeSimulateConfig | undefined;
  ...
  // 默认配置值
  const getDefaultConfig = useCallback((prev?: VariableNodeSimulateConfig): VariableNodeSimulateConfig => ({
  ...
  // 通用的字段更新方法
  const updateField = useCallback(<K extends keyof VariableNodeSimulateConfig>(
    field: K, 
