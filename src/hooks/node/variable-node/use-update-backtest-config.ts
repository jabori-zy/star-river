...
export const useUpdateBacktestConfig = ({ id, initialConfig }: UseUpdateBacktestConfigProps) => {
  ...
  // 统一的状态管理
  const [config, setConfig] = useState<VariableNodeBacktestConfig | undefined>(initialConfig);
  ...
  // 通用的更新函数
  const updateConfig = useCallback((updater: (prev: VariableNodeBacktestConfig | undefined) => VariableNodeBacktestConfig) => {
    // 获取最新的节点数据，而不是依赖可能过时的state
    const currentNode = getNode(id);
    const currentConfig = currentNode?.data?.backtestConfig as VariableNodeBacktestConfig | undefined;
  ...
  // 默认配置值
  const getDefaultConfig = useCallback((prev?: VariableNodeBacktestConfig): VariableNodeBacktestConfig => ({
  ...
  // 通用的字段更新方法
  const updateField = useCallback(<K extends keyof VariableNodeBacktestConfig>(
    field: K, 
  ...
  // 更新交易所模式配置
  const updateExchangeModeConfig = useCallback((exchangeModeConfig: VariableNodeBacktestExchangeModeConfig | undefined) => {
  ...
  // 更新变量配置列表
  const updateVariableConfigs = useCallback((variableConfigs: VariableConfig[]) => {
    updateField('variableConfigs', variableConfigs);
  }, [updateField]);

  // 添加变量配置
  const addVariableConfig = useCallback((variableConfig: Omit<VariableConfig, 'configId'>) => {
    updateConfig(prev => {
      const currentConfigs = prev?.variableConfigs || [];
      const newId = Math.max(0, ...currentConfigs.map(config => config.configId)) + 1;
  ...
  // 更新指定变量配置
  const updateVariableConfig = useCallback((index: number, variableConfig: VariableConfig) => {
    updateConfig(prev => {
      const currentConfigs = prev?.variableConfigs || [];
  ...
  // 删除变量配置
  const removeVariableConfig = useCallback((index: number) => {
    updateConfig(prev => {
      const currentConfigs = prev?.variableConfigs || [];
  ...
  // 根据ID删除变量配置
  const removeVariableConfigById = useCallback((configId: number) => {
    updateConfig(prev => {
      const currentConfigs = prev?.variableConfigs || [];
      const updatedConfigs = currentConfigs.filter(config => config.configId !== configId);
