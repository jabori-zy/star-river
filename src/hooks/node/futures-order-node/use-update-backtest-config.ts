...
export const useUpdateBacktestConfig = ({ id, initialConfig }: UseUpdateBacktestConfigProps) => {
  ...
  // 统一的状态管理
  const [config, setConfig] = useState<FuturesOrderNodeBacktestConfig | undefined>(initialConfig);
  ...
  // 通用的更新函数
  const updateConfig = useCallback((updater: (prev: FuturesOrderNodeBacktestConfig | undefined) => FuturesOrderNodeBacktestConfig) => {
    // 获取最新的节点数据，而不是依赖可能过时的state
    const currentNode = getNode(id);
    const currentConfig = currentNode?.data?.backtestConfig as FuturesOrderNodeBacktestConfig | undefined;
