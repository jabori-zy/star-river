...
export const useUpdateSimulateConfig = ({ id, initialConfig }: UseUpdateSimulateConfigProps) => {
  ...
  // 统一的状态管理
  const [config, setConfig] = useState<FuturesOrderNodeSimulateConfig | undefined>(initialConfig);
  ...
  // 通用的更新函数
  const updateConfig = useCallback((updater: (prev: FuturesOrderNodeSimulateConfig | undefined) => FuturesOrderNodeSimulateConfig) => {
    // 获取最新的节点数据，而不是依赖可能过时的state
    const currentNode = getNode(id);
    const currentConfig = currentNode?.data?.simulateConfig as FuturesOrderNodeSimulateConfig | undefined;
  ...
  // 默认配置值
  const getDefaultConfig = useCallback((prev?: FuturesOrderNodeSimulateConfig): FuturesOrderNodeSimulateConfig => ({
  ...
  // 通用的字段更新方法
  const updateField = useCallback(<K extends keyof FuturesOrderNodeSimulateConfig>(
    field: K, 
