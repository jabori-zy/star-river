...
import { BacktestStrategyEvent, klineUpdateEvent, indicatorUpdateEvent, FuturesOrderFilledEvent } from "@/types/strategy-event/backtest-strategy-event";
...
class BacktestStrategyDataObservableService {
    ...
    private connectionState$ = new BehaviorSubject<SSEConnectionState>(SSEConnectionState.DISCONNECTED);
    ...
    createKlineStream(enabled: boolean = true): Observable<klineUpdateEvent> {
        ...
        if (this.eventSource && this.eventSource.readyState === EventSource.OPEN) {
            return this.klineDataSubject.asObservable().pipe(
    ...
    createIndicatorStream(enabled: boolean = true): Observable<indicatorUpdateEvent> {
        ...
        // 如果已经连接，直接返回现有的数据流
        if (this.eventSource && this.eventSource.readyState === EventSource.OPEN) {
            return this.indicatorDataSubject.asObservable().pipe(
                takeUntil(this.destroy$),
                share()
            );
        }
     ...
     */
    createOrderStream(enabled: boolean = true): Observable<FuturesOrderFilledEvent> {
        if (!enabled) {
            this.disconnect();
            return new Observable(subscriber => {
                // 返回空的Observable
        ...
        // 如果已经连接，直接返回现有的数据流
        if (this.eventSource && this.eventSource.readyState === EventSource.OPEN) {
            return this.orderDataSubject.asObservable().pipe(
                takeUntil(this.destroy$),
                share()
            );
        }
     ...
     * @returns 过滤后的订单数据流
     */
    createOrderStreamForSymbol(exchange: string, symbol: string, enabled: boolean = true): Observable<VirtualOrder> {
...
// 创建单例实例
const backtestStrategyDataObservableService = new BacktestStrategyDataObservableService();
...
export const createOrderStreamForSymbol = (exchange: string, symbol: string, enabled: boolean = true) =>
    backtestStrategyDataObservableService.createOrderStreamForSymbol(exchange, symbol, enabled);
