...
import { MAValue, SMAValue, EMAValue, BBandsValue, MACDValue } from "./indicator-value";
...
// 综合指标配置映射
export const COMPREHENSIVE_INDICATOR_CONFIG: Record<IndicatorType, ComprehensiveIndicatorConfig> = {
    [IndicatorType.MA]: {
        ...
        params: [
            ...
            },
            {
                ...
                required: true,
                options: Object.values(MAType).map(type => ({ label: type, value: type }))
            },
            {
                label: "数据源",
                name: "priceSource",
...
// 工具函数：根据指标类型获取配置
export function getIndicatorConfig(type: IndicatorType): ComprehensiveIndicatorConfig {
...
// 工具函数：获取参数默认值
export function getParamDefaultValue(type: IndicatorType, paramName: string): string | number | PriceSource | MAType | undefined {
    const config = COMPREHENSIVE_INDICATOR_CONFIG[type];
...
// 工具函数：验证指标参数
export function validateIndicatorParams(type: IndicatorType, params: Record<string, unknown>): boolean {
    const config = COMPREHENSIVE_INDICATOR_CONFIG[type];
