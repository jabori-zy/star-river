...
// 状态列表
const statusOptions: Array<"normal" | "warning" | "error" | "inactive"> = [
	"normal",
	"warning",
	"error",
	"inactive",
...
function generateMT5Accounts(count: number): MT5Account[] {
	...
	for (let i = 0; i < count - mt5AccountsBase.length; i++) {
		...
		const randomServer =
			mt5Servers[Math.floor(Math.random() * mt5Servers.length)];
		const randomStatus =
	...
	for (let i = 0; i < count - binanceAccountsBase.length; i++) {
		...
		const randomPermissions =
			permissionCombinations[
				Math.floor(Math.random() * permissionCombinations.length)
			];
		const randomStatus =
			statusOptions[Math.floor(Math.random() * statusOptions.length)];
		const randomDate = generateRandomDate();
...
function generateOKXAccounts(count: number): OKXAccount[] {
	...
	for (let i = 0; i < count - okxAccountsBase.length; i++) {
		...
		const randomAvailableBalance =
			randomTotalAssets * (0.7 + Math.random() * 0.25); // 可用余额在总资产的70%-95%之间
