...
const StatusSection: React.FC<StatusSectionProps> = ({
    ...
    const latestEventInfo = useMemo(() => {
        ...
        if ('strategyState' in latestLog) {
            ...
            const isFailed = strategyLog.strategyState === StrategyState.Failed || 
            ...
            // 2. 或者有错误代码且日志级别为 error
            const isFailed = nodeLog.nodeState === NodeState.Failed || 
        ...
        switch (currentStage) {
                    ...
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            {latestEventInfo.statusText && (
                                <div className={`text-sm ${latestEventInfo.isFailed ? 'text-red-600' : 'text-gray-600'}`}>
            ...
            case "completed":
                return (
                    <div className="flex items-center space-x-3">
                        <CheckCircle className="w-5 h-5 text-green-500" />
                        <div className="font-medium text-green-700">策略加载完成，准备启动...</div>
                    </div>
                );
            case "failed":
                return (
