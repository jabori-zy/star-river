...
interface LiveAccountProps {
  ...
  setLiveAccounts: (accounts: AccountItem[]) => void;
}
...
export const LiveAccount = ({ liveAccounts, setLiveAccounts }: LiveAccountProps) => {
  ...
  // 更新账户信息
  const updateLocalAccount = (index: number, updates: Partial<AccountItem>) => {
    const newAccounts = [...localAccounts];
  ...
  // 获取实盘账户配置列表
  const getMT5AccountConfigs = async () => {
    try {
  ...
  // 点击刷新按钮时获取最新账户列表
  const handleFetchMT5Accounts = async () => {
    try {
  ...
  const handleAccountChange = (index: number, selectedId: string) => {
    ...
    if (selectedAccount) {
      updateLocalAccount(index, { 
        ...
        exchange: selectedAccount.exchange === "metatrader5" ? "MT5" : selectedAccount.exchange,
      });
      setErrorMessage("");
    }
  };
  ...
  // 渲染账户选择器
  const renderAccountSelector = (account: AccountItem, index: number) => {
