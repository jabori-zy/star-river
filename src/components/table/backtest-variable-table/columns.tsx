...
export const createStrategyVariableColumns = (
	...
	},
	{
		...
		enableSorting: false,
		accessorFn: (row) =>
			isCustomVariableUpdateEvent(row) ? "" : (row.sysVariable.symbol ?? ""),
		cell: ({ row }) => {
	...
	},
	{
		...
		enableSorting: false,
		accessorFn: (row) =>
			isCustomVariableUpdateEvent(row)
				? renderValue(
						row.customVariable.initialValue,
						row.customVariable.varValueType,
					)
				: "",
		cell: ({ row }) => {
			const value = isCustomVariableUpdateEvent(row.original)
				? renderValue(
						row.original.customVariable.initialValue,
						row.original.customVariable.varValueType,
					)
				: "";
			return (
				<div className="text-left truncate text-sm" title={value}>
	...
	},
	{
		...
		cell: ({ row }) => (
			<div className="text-left truncate text-sm" title={row.original.datetime}>
