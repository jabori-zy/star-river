...
export function generateMockVirtualOrders(count: number = 50): VirtualOrder[] {
	...
	const orderSides: FuturesOrderSide[] = [
		FuturesOrderSide.OPEN_LONG,
		FuturesOrderSide.OPEN_SHORT,
		FuturesOrderSide.CLOSE_LONG,
		FuturesOrderSide.CLOSE_SHORT,
	];
	...
	for (let i = 0; i < count; i++) {
		...
		const updateTime = new Date(
			new Date(createTime).getTime() + Math.random() * 24 * 60 * 60 * 1000,
		).toISOString();
