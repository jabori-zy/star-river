...
const SymbolSelector: React.FC<SymbolSelectorProps> = ({ 
    ...
    const [editingSymbol, setEditingSymbol] = useState<SelectedSymbol | undefined>(undefined);
    ...
    const handleDeleteSymbol = (symbolToDelete: SelectedSymbol) => {
        const newSymbols = localSymbols.filter(s => 
            !(s.symbol === symbolToDelete.symbol && s.interval === symbolToDelete.interval)
        );
    ...
    const handleSave = () => {
        ...
        if (editingSymbol) {
            ...
            newSymbols = localSymbols.map(s => 
                (s.symbol === editingSymbol.symbol && s.interval === editingSymbol.interval) ? newSymbol : s
            );
        } else {
            // 添加新交易对
    ...
    const getIntervalLabel = (interval: string) => {
        return TIME_INTERVALS.find(item => item.value === interval)?.label || interval;
    };
            ...
            <div className="space-y-2">
                {localSymbols.length === 0 ? (
                    ...
                    localSymbols.map((symbol, index) => (
                        <div key={`${symbol.symbol}-${symbol.interval}-${index}`} className="flex items-center justify-between p-2 border rounded-md bg-background group">
                            <div className="flex items-center gap-2">
                                ...
                                <div className="flex items-center gap-1">
                                    ...
                                    <span className="text-sm">{getIntervalLabel(symbol.interval)}</span>
                                </div>
                            </div>
                            <div className="flex items-center gap-1">
