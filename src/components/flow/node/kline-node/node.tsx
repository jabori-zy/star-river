...
const KlineNode: React.FC<NodeProps<KlineNodeType>> = ({id, data, selected}) => {
    ...
    const { updateTimeRange, setDefaultBacktestConfig} = useUpdateBacktestConfig({ id, initialBacktestConfig: data?.backtestConfig });
    const { setDefaultLiveConfig} = useUpdateLiveConfig({ id, initialLiveConfig: data?.liveConfig });
