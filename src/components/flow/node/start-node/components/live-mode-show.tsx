                            ...
                            <CollapsibleContent className="mt-2">
                                <div>
                                    {liveConfig.liveAccounts.map((account: SelectedAccount) => {
                                        return (
                                            <div key={account.id} className="flex items-center gap-2 bg-gray-100 p-2 rounded-md">
                                                <span className="text-sm"> {account.accountName} ({account.exchange})</span>
                                            </div>
                                        )
                                    })}
                                </div>
            ...
            <div className="space-y-2">
                {
                    liveConfig.variables?.length === 0 ? (
                        ...
                        <Collapsible open={isVariablesOpen} onOpenChange={setIsVariablesOpen}>
                            ...
                            <CollapsibleContent className="mt-2">
                                <div className="space-y-2">
                                    {liveConfig.variables?.map((variable: StrategyVariable, index: number) => {
                                        return (
