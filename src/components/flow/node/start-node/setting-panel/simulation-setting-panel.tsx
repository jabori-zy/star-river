...
export const StartNodeSimulationSettingPanel: React.FC<SettingProps> = ({ id, data }) => {
    ...
    // 模拟模式配置
    const [simulateConfig, setSimulateConfig] = useState<StrategySimulateConfig | undefined>(startNodeData.simulateConfig as StrategySimulateConfig);
    // 已选择账户列表
    const [selectedAccounts, setSelectedAccounts] = useState<SelectedAccount[]>(simulateConfig?.simulateAccounts || []);
    // 变量列表
    const [variables, setVariables] = useState<StrategyVariable[]>(simulateConfig?.variables || []);

    // 更新已选择账户
    const handleUpdateSelectedAccounts = useCallback((accounts: SelectedAccount[]) => {
    ...
    // 更新变量列表
    const handleVariablesChange = useCallback((newVariables: StrategyVariable[]) => {
