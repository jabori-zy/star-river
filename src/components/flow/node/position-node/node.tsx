...
import type { PositionNode as PositionNodeType } from "@/types/node/position-node";
...
> = ({ id, selected }) => {
	...
	const iconName = currentNodeData?.nodeConfig?.iconName || getNodeIconName(NodeType.PositionNode);
	const iconBackgroundColor = currentNodeData?.nodeConfig?.iconBackgroundColor || getNodeDefaultColor(NodeType.PositionNode);
	const borderColor = currentNodeData?.nodeConfig?.borderColor || getNodeDefaultColor(NodeType.PositionNode);
	const handleColor = currentNodeData?.nodeConfig?.handleColor || getNodeDefaultColor(NodeType.PositionNode);
