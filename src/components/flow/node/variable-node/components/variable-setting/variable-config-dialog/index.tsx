...
const VariableConfigDialog: React.FC<VariableConfigDialogProps> = ({
	...
	const [triggerCase, setTriggerCase] = React.useState<TriggerCase | null>(
		null,
	);
			...
			if (isEditing && editingConfig) {
				...
				if (editingConfig.varOperation === "get") {
					...
					if (editingConfig.timerConfig) {
						setTimerConfig(editingConfig.timerConfig);
					}
					// 恢复条件触发配置
					if (editingConfig.triggerCase) {
						setTriggerCase(editingConfig.triggerCase);
					}
					...
					// 根据触发方式恢复不同的值
					if (editingConfig.varTriggerType === "dataflow") {
						// 数据流模式：恢复 DataFlow 对象
						const dataflowValue =
							editingConfig.updateOperationValue as TriggerDataFlow;
						setDataflowNodeId(dataflowValue.fromNodeId);
						setDataflowHandleId(dataflowValue.fromHandleId);
						setDataflowVariable(dataflowValue.fromVar);
						setDataflowVariableName(dataflowValue.fromVarDisplayName);
						// 从 variableItemList 中查找对应的节点类型和名称
						const selectedNode = variableItemList.find(
							(item) => item.nodeId === dataflowValue.fromNodeId,
						);
						if (selectedNode) {
	...
	const handleSave = () => {
		...
		const timerConfigData: TimerConfig | undefined =
		...
		if (varOperation === "get") {
			...
			let updateOperationValue: string | number | boolean | TriggerDataFlow;
			...
			const updateConfig: UpdateVariableConfig = {
				...
				triggerCase: updateTriggerType === "condition" ? triggerCase : null,
			...
			const resetConfig: ResetVariableConfig = {
				...
				outputHandleId: editingConfig?.outputHandleId || "",
				varOperation: "reset",
				varTriggerType: triggerType,
				timerConfig: timerConfigData,
				triggerCase: triggerType === "condition" ? triggerCase : null,
