...
import { TradeMode, BacktestDataSource, SelectedAccount } from "@/types/strategy";
import VariableSetting from "../components/variable-setting";
import { useUpdateBacktestConfig } from "@/hooks/node/variable-node";
import { VariableNodeBacktestConfig, VariableConfig } from "@/types/node/variable-node";




const VariableNodeBacktestSettingPanel: React.FC<SettingProps> = ({ id, data }) => {
    const {
        config,
        ...
        id, 
        initialConfig: data?.backtestConfig as VariableNodeBacktestConfig | undefined
    });
    ...
    // 获取当前选中的账户
    const selectedAccount = config?.dataSource === BacktestDataSource.EXCHANGE 
