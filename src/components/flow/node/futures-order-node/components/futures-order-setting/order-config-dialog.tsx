...
const OrderConfigDialog: React.FC<OrderConfigDialogProps> = ({
    ...
    const handleSave = () => {
        ...
        if ((orderType === OrderType.LIMIT || orderType === OrderType.STOP_LIMIT || orderType === OrderType.TAKE_PROFIT_LIMIT) && price <= 0) {
            return;
        }
    ...
    const isMarketOrder = orderType === OrderType.MARKET || orderType === OrderType.STOP_MARKET;

    return (
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                    <DialogTitle>{isEditing ? '配置期货订单' : '添加期货订单'}</DialogTitle>
                    <DialogDescription>
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                        ...
                        <div className="col-span-3">
                            <Input
                                ...
                                type="number"
                                value={tp || ''}
                                onChange={(e) => setTp(e.target.value ? Number(e.target.value) : null)}
                                min={0}
                                step={0.01}
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                        ...
                        <div className="col-span-3">
                            <Input
                                ...
                                type="number"
                                value={sl || ''}
                                onChange={(e) => setSl(e.target.value ? Number(e.target.value) : null)}
                                min={0}
                                step={0.01}
                ...
                <DialogFooter>
                    ...
                    <Button 
                        onClick={handleSave}
                        disabled={!symbol.trim() || quantity <= 0 || (!isMarketOrder && price <= 0)}
                    >
                        保存
                    </Button>
                </DialogFooter>
