...
import { AtSign, Variable, ChevronRight, ChevronDown, Wallet, TrendingUp, PercentSquare, Play } from "lucide-react";
            ...
            <div className="space-y-2">
                <Collapsible open={isSettingsOpen} onOpenChange={setIsSettingsOpen}>
                    <CollapsibleTrigger className="flex items-center gap-2 w-full">
                        ...
                        <Label className="text-xm font-bold text-muted-foreground">回测设置</Label>
                    </CollapsibleTrigger>
                    <CollapsibleContent className="mt-2">
                        <div className="flex flex-col gap-2 bg-gray-100 rounded-md p-2">
                            ...
                            <div className="flex items-center justify-between gap-2 rounded-md ">
                                ...
                                <span className="text-sm  font-bold">{backtestConfig.initialBalance}</span>
                            </div>
                            {/* 杠杆倍数 */}
                            <div className="flex items-center justify-between gap-2 rounded-md ">
                                <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm  font-bold">{backtestConfig.leverage}x</span>
                            </div>
                            {/* 手续费率 */}
                            <div className="flex items-center justify-between gap-2 rounded-md ">
                                <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm  font-bold">{(backtestConfig.feeRate * 100).toFixed(2)}%</span>
                            </div>
                            {/* 播放速度 */}
                            <div className="flex items-center justify-between gap-2 rounded-md ">
                                <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm  font-bold">{backtestConfig.playSpeed}x</span>
                            </div>
                        </div>
                    </CollapsibleContent>
                </Collapsible>
            ...
            <div className="space-y-2">
                {
                    !backtestConfig.variables || backtestConfig.variables.length === 0 ? (
                        <div className="flex items-center justify-between gap-2 rounded-md">
                            <Label className="text-xm font-bold text-muted-foreground">变量</Label>
                        ...
                        <Collapsible open={isVariablesOpen} onOpenChange={setIsVariablesOpen}>
                            ...
                            <CollapsibleContent className="mt-2">
                                <div className="space-y-2">
                                    {backtestConfig.variables.map((variable: StrategyVariable, index: number) => {
                                        return (
                                            <div key={index} className="flex flex-col bg-gray-100 p-2 rounded-md">
                                                {variable.varType === "number" && (
                                                    <div className="flex items-center gap-2" title={variable.varName}>
                                                        <Variable className="w-4 h-4 text-green-500" />
                                                        <span className="text-sm"> {variable.varName} = {variable.varValue}</span>
                                                    </div>
                                                )}
