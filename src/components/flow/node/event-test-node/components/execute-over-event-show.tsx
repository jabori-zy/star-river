...
import { ActivitySquare, Calendar } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { ScrollArea } from "@/components/ui/scroll-area";
import type { ReceiveExecuteOverEvent } from "@/types/strategy-event/event-received-event";

interface ExecuteOverEventStats {
...
interface ExecuteOverEventShowProps {
	events: ReceiveExecuteOverEvent[];
}

export function ExecuteOverEventShow({ events }: ExecuteOverEventShowProps) {
	// Group events by configId + eventName and calculate statistics
	const stats = useMemo(() => {
		if (events.length === 0) return [];

		const statsMap = new Map<string, ExecuteOverEventStats>();

		for (const event of events) {
			const key = `${event.configId ?? "none"}-${event.eventName}`;
			...
			<div className="pb-2 px-3 pt-2">
				<div className="flex items-center justify-between gap-2">
					<div className="flex items-center gap-1.5 text-xs font-semibold">
						...
						<span className="whitespace-nowrap">Execute Over Events</span>
					</div>
			...
			<div className="pb-2 px-3">
				{stats.length === 0 ? (
					<div className="text-center text-xs text-muted-foreground py-4">
						No execute over events received yet
					</div>
				) : (
					<>
