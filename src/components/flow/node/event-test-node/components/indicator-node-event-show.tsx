...
import type { ReceiveIndicatorNodeEvent } from "@/types/strategy-event/event-received-event";
import { parseKey } from "@/utils/parse-key";
import type { IndicatorKey } from "@/types/symbol-key";

interface IndicatorNodeEventStats {
	configId: number;
	eventName: string;
	symbol: string;
	indicatorSeriesLength: number;
	latestCycleId: number;
	receiveCount: number;
	lastDatetime: string;
}

interface IndicatorNodeEventShowProps {
	events: ReceiveIndicatorNodeEvent[];
	sourceHandleId: string | null;
}

export function IndicatorNodeEventShow({ events, sourceHandleId }: IndicatorNodeEventShowProps) {
	...
	const stats = useMemo(() => {
		...
		const statsMap = new Map<string, IndicatorNodeEventStats>();

		for (const event of filteredEvents) {
			const key = `${event.configId}-${event.eventName}`;

			if (!statsMap.has(key)) {
				// Parse indicatorKey to get symbol
				let symbol = "Unknown";
				try {
					const parsedKey = parseKey(event.indicatorKey) as IndicatorKey;
					symbol = parsedKey.symbol;
				} catch (error) {
					console.error("Failed to parse indicatorKey:", event.indicatorKey, error);
				}
				...
				statsMap.set(key, {
					...
					symbol: symbol,
					indicatorSeriesLength: event.indicatorSeriesLength,
			...
			<div className="pb-2 px-3 pt-2">
				<div className="flex items-center justify-between gap-2">
					<div className="flex items-center gap-1.5 text-xs font-semibold">
						...
						<span className="whitespace-nowrap">Indicator Node Events</span>
					</div>
				</div>
			</div>
			...
			<div className="pb-2 px-3">
				{stats.length === 0 ? (
					<div className="text-center text-xs text-muted-foreground py-4">
						No indicator node events received yet
					</div>
				) : (
					<>
								...
								{stats.map((stat, index) => (
									...
									>
										...
										<div className="flex items-center gap-1.5 text-[10px] text-muted-foreground mb-1">
											...
											<span>Length: {stat.indicatorSeriesLength}</span>
										</div>
