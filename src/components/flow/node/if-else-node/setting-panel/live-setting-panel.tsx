...
const IfElseNodeLiveSettingPanel: React.FC<SettingProps> = ({ id, data }) => {
	...
	const [localLiveCases, setLocalLiveCases] = useState<CaseItem[]>(
		ifElseNodeData?.liveConfig?.cases || [],
	);

	const { updateCase, removeCase, updateCases } = useUpdateLiveConfig({
		id,
		initialConfig: ifElseNodeData?.liveConfig,
	});
