...
const KlineNodePanel = ({
  ...
  const onSave = () => {
    try {
      console.log('开始保存配置，交易模式:', tradingMode);
      ...
      if (tradingMode === TradeMode.LIVE || data.liveConfig) {
        // 安全解析已选择的账户
        let selectedLiveAccountObj;
        try {
          selectedLiveAccountObj = liveSelectedAccount ? JSON.parse(liveSelectedAccount) : null;
        } catch (e) {
          console.warn('解析实盘账户数据失败:', e);
          selectedLiveAccountObj = null;
        }
      ...
      if (tradingMode === TradeMode.SIMULATE || data.simulateConfig) {
        ...
        try {
          selectedAccountObj = simulateSelectedAccount ? JSON.parse(simulateSelectedAccount) : null;
        } catch (e) {
          console.warn('解析模拟账户数据失败:', e);
      ...
      if (tradingMode === TradeMode.BACKTEST || data.backtestConfig) {
        ...
        if (backtestDataSource === BacktestDataSource.EXCHANGE) {
          // 安全解析数据源
          let selectedDataSourceObj;
          try {
            selectedDataSourceObj = selectedDataSource ? JSON.parse(selectedDataSource) : undefined;
          } catch (e) {
            console.warn('解析数据源配置失败:', e);
            selectedDataSourceObj = undefined;
          }
