...
export function IndicatorLegendEditDialog({
  ...
  const [originalSeriesConfigs, setOriginalSeriesConfigs] = useState<SeriesConfig[]>([]);
    ...
    if (open) {
      ...
      if (chart) {
        const indicatorConfig = chart.indicatorChartConfigs.find(
          (config) => config.indicatorKeyStr === indicatorKeyStr && !config.isDelete
        );
        if (indicatorConfig) {
      ...
      <DialogContent className="sm:max-w-[350px]">
        ...
        <div className="space-y-3 py-4">
          {tempSeriesConfigs.map((series, index) => (
            <div key={`${series.name}-${series.indicatorValueKey}-${index}`} className="flex items-center justify-between">
              {/* 系列名称 */}
