...
export const getCursorTooltipLegendTemplate = (seriesInfos: SeriesInfo[], _svgAnnotation: CursorTooltipSvgAnnotation) => {
    let outputSvgString = "";
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
export const getRolloverLegendTemplate = (seriesInfos: SeriesInfo[], _svgAnnotation: RolloverLegendSvgAnnotation) => {
    let outputSvgString = "";
    ...
    // 更新最新价格注释的位置和颜色
export const updateLatestPriceAnnotation = (latestPriceAnnotation: HorizontalLineAnnotation, kline: Kline) => {
        latestPriceAnnotation.isHidden = false;
        latestPriceAnnotation.y1 = kline.close;
        latestPriceAnnotation.stroke = kline.close > kline.open ? appTheme.MutedRed : appTheme.MutedGreen;
...
export const handleFirstKlineData = (
...
): { firstCandleTimestamp: number; firstKlineHighPrice: number; firstKlineLowPrice: number } => {
    const { candleDataSeries, volumeDataSeries, xAxis, yAxis, candleInterval } = context;
    const timestamp = kline.timestamp / 1000;
    
    // 添加数据
    candleDataSeries.append(timestamp, kline.open, kline.high, kline.low, kline.close);
    ...
    console.log(`第一根K线添加完成，设置初始x轴范围: ${new Date(startTime * 1000).toLocaleString()} - ${new Date(endTime * 1000).toLocaleString()}`);
    console.log(`第一根K线添加完成，设置初始y轴范围: ${yMin.toFixed(2)} - ${yMax.toFixed(2)}`);
...
export const adjustYAxisRange = (
    ...
    console.log(`${isUpdate ? '更新' : '添加'}K线-当前y轴范围: ${currentYRange.min.toFixed(2)} - ${currentYRange.max.toFixed(2)}, K线价格: ${klineLow.toFixed(2)} - ${klineHigh.toFixed(2)}`);
    ...
    if (klineHigh > currentYRange.max || klineLow < currentYRange.min) {
        ...
        console.log(`${isUpdate ? '更新' : '添加'}K线-调整y轴范围: ${newMin.toFixed(2)} - ${newMax.toFixed(2)}, K线价格: ${klineLow.toFixed(2)} - ${klineHigh.toFixed(2)}`);
    }
};
...
export const adjustXAxisRangeForFewCandles = (
    ...
    console.log(`K线数量: ${candleCount}, 重新计算范围: ${new Date(startTime * 1000).toLocaleString()} - ${new Date(endTime * 1000).toLocaleString()}`);
};
...
export const handleUpdateKlineData = (
    ...
    const timestamp = kline.timestamp / 1000;
    const latestCandleTimestamp = candleDataSeries.getNativeXValues().get(currentIndex);
    
    console.log(`更新K线: 时间戳=${timestamp}, 最后K线时间戳=${latestCandleTimestamp}, K线数量=${candleDataSeries.count()}`);
...
export const handleNewKlineData = (
    ...
    const { candleDataSeries, volumeDataSeries, yAxis, xAxis, candleInterval, maxVisibleCandles, firstCandleTimestamp } = context;
    const timestamp = kline.timestamp / 1000;
    ...
    // 根据K线数量调整X轴范围
    if (candleCount <= 10) {
        adjustXAxisRangeForFewCandles(candleCount, timestamp, firstCandleTimestamp!, candleInterval, xAxis);
    } else {
        adjustXAxisRangeForManyCandles(candleCount, timestamp, latestCandleTimestamp, candleInterval, maxVisibleCandles, xAxis);
    }
};
...
export const processKlineData = (
    ...
    console.log(`K线时间戳: ${timestamp}, 格式化时间: ${new Date(timestamp * 1000).toLocaleString()}`);
    console.log(`K线数据: 开盘=${kline.open}, 高=${kline.high}, 低=${kline.low}, 收盘=${kline.close}`);
    ...
    if (candleDataSeries.count() === 0) {
        ...
        console.log("最后一条蜡烛的时间戳", new Date(latestCandleTimestamp * 1000).toLocaleString());
