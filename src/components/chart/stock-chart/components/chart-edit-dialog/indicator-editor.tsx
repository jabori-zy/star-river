...
const IndicatorEditor = ({ mode, klineChartConfig, subChartConfigs }: IndicatorEditorProps) => {
    return (
        <div className="space-y-4">
            ...
            {mode === "main" && klineChartConfig && (
                <div className="space-y-3">
                    {Object.entries(klineChartConfig.indicatorChartConfig).map(([cacheKey, config]) => (
                        <div key={cacheKey} className="bg-gray-50 p-4 rounded-lg">
                            ...
                            <div className="space-y-2 text-sm">
                                <div>
                                    ...
                                    <span className="font-mono text-xs bg-gray-200 px-1 rounded">{cacheKey}</span>
                                </div>
                                <div>
                                    <span className="text-gray-600">位置: </span>
                                    <span className={config.isInMainChart ? "text-blue-600" : "text-orange-600"}>
                                ...
                                <div className="mt-2">
                                    ...
                                    {config.seriesConfigs.map((series, index) => (
                                        <div key={index} className="ml-2 text-xs bg-white p-2 rounded border">
            ...
            {mode === "sub" && subChartConfigs && (
                <div className="space-y-4">
                    {subChartConfigs.map((subConfig, subIndex) => (
                        <div key={subConfig.subChartId} className="border border-gray-200 rounded-lg p-4">
                            <h4 className="font-medium text-gray-900 mb-3">
                                子图 {subIndex + 1} (ID: {subConfig.subChartId})
                            </h4>

                            <div className="space-y-3">
                                {Object.entries(subConfig.indicatorChartConfigs).map(([cacheKey, config]) => (
                                    <div key={cacheKey} className="bg-gray-50 p-3 rounded">
                                        <h5 className="font-medium text-gray-800 mb-2">{config.name}</h5>
