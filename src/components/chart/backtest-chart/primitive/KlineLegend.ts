...
class KlineLegendRenderer implements IPrimitivePaneRenderer {
	...
	private _renderInBitmapSpace(
		target: CanvasRenderingTarget2D,
		data: KlineLegendData,
	) {
		target.useBitmapCoordinateSpace((scope) => {
			const ctx = scope.context;
			const { textColor, fontSize, padding, position, upColor, downColor } =
				this._options;
			...
			// 计算每个段的宽度
			let currentX =
				position.left * scope.horizontalPixelRatio +
				padding * scope.horizontalPixelRatio;
			const y =
				position.top * scope.verticalPixelRatio +
				padding * scope.verticalPixelRatio;
			...
			if (this._isHovered) {
				...
				if (this._options.borderRadius > 0) {
					const radius =
						this._options.borderRadius * scope.horizontalPixelRatio;
					ctx.beginPath();
