...
export function virtualOrderToMarker(virtualOrder: VirtualOrder): OrderMarker[] {
    ...
    const timestampInSeconds = getChartAlignedUtcTimestamp(virtualOrder.updateTime) as UTCTimestamp;
 ...
 */
export const getChartAlignedUtcTimestamp = (input: DateTime | string): number => {
 ...
 */
export const getDateTimeFromChartTimestamp = (chartTimestamp: number): string | null => {
	// 1. 将秒转换为毫秒
