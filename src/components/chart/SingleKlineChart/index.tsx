		...
		if (series && series.visible) {
			...
			if (series.points.length === 0) {
				...
				// 时间戳相同，更新现有点
					...
					const currentData =
						(
							series.options as Highcharts.SeriesOptionsType & {
								data: number[][];
							}
						).data || [];
...
// 更新K线数据的函数
function updateKlineData(chart: Highcharts.Chart, values: number[][]): void {
	// 最新的数据
	...
	if (klineSeries && klineSeries.visible) {
		...
		if (klineSeries.points.length === 0) {
			...
			// 时间戳相同，更新现有点
				...
				const currentKlineSeriesData =
					(
						klineSeries.options as Highcharts.SeriesOptionsType & {
							data: number[][];
						}
					).data || [];
				currentKlineSeriesData[currentKlineSeriesData.length - 1] =
					newKlinePoint;
				klineSeries.setData(currentKlineSeriesData);
				const currentVolumeSeriesData =
					(
						volumeSeries.options as Highcharts.SeriesOptionsType & {
							data: number[][];
						}
					).data || [];
				currentVolumeSeriesData[currentVolumeSeriesData.length - 1] =
					newVolumePoint;
...
function SingleKlineChart(lineChartProps: SingleKlineChartProps) {
	...
	// 获取最新事件
	const latestStrategyEvent = useStrategyEventStore((state) =>
		state.getLatestEvent("strategy-data-update"),
	);
	...
	useEffect(() => {
		console.log(
			"图表当前的cacheKey",
			lineChartProps.klineCacheKey,
			lineChartProps.indicatorCacheKeys,
		);
		// 获取初始k线数据
		const fetchInitialData = async () => {
			// 获取k线缓存数据
			const initialKlineData = await getInitialChartData(
				lineChartProps.klineCacheKey,
			);
	...
	useEffect(() => {
		...
		if (
			!canProcessUpdates ||
			!latestStrategyEvent ||
			lineChartProps.klineCacheKey.length === 0
		)
			return;
