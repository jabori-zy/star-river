...
// 设置K线数据
export function setKlineData(
	options: Highcharts.Options,
	klineData: number[][],
	klineCacheKeyStr: KlineKeyStr,
): Highcharts.Options {
	...
	// 检查是否已存在K线系列
	const klineSeriesIndex = newOptions.series?.findIndex(
		(s) => s.id === "kline",
	);
	const volumeSeriesIndex = newOptions.series?.findIndex(
		(s) => s.id === "volume",
	);
...
// 设置指标数据
export function setIndicatorData(
	options: Highcharts.Options,
	indicatorData: Record<IndicatorKeyStr, number[][]>,
): Highcharts.Options {
	const newOptions = { ...options };
	if (!newOptions.series) {
		newOptions.series = [];
	...
	Object.keys(indicatorData).forEach((key) => {
		...
		if (indicatorSeriesConfig) {
			// 检查是否已存在该指标系列
			const seriesIndex = newOptions.series?.findIndex(
				(s) => s.id === indicatorSeriesConfig.id,
			);
