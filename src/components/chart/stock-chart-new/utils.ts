...
export function updateSplitterPositions() {
    const splitters = Array.from(document.querySelectorAll(".chart-splitter")) as HTMLElement[];
    let currentY = 0;
...
// 设置splitter事件
export function setupSplitterEvents(splitter: HTMLElement, splitterIndex: number) {
    ...
    document.addEventListener("pointermove", (e) => {
        ...
        if (mouseY >= minY && mouseY <= maxY) {
            ...
            const upperPanelStart = upperPanelIndex > 0 
...
export function addSubChart() {
    ...
    if (chartCount > 0) {
        ...
        const splitter = createSplitter(`splitter${chartCount - 1}`, splitterPosition);
