...
export type TFinanceLegendTemplate = (
	legendAnnotation: FinChartLegendAnnotation,
) => string;

export interface IFinChartLegendAnnotationOptions
	extends ISvgAnnotationBaseOptions {
...
export class FinChartLegendAnnotation extends SvgAnnotationBase {
	...
	private templateProperty: TFinanceLegendTemplate =
		defaultFinanceLegendTemplate;
	 ...
	 */
	constructor(
		sciFinanceChart: SciChartSurface,
		options?: IFinChartLegendAnnotationOptions,
	) {
		super(options);
	...
	protected create(
		...
		if (this.template) {
			this.xIndex = this.isMouseOverSeriesArea
				? Math.round(xCalc.getDataValue(this.x1))
				: undefined;
...
/** @ignore */
const defaultFinanceLegendTemplate: TFinanceLegendTemplate = (
	la: FinChartLegendAnnotation,
): string => {
	const outputStrings: string[] = [];
	const subSurface = la.sciFinanceChart.subCharts.find(
		(study) => study.id === la.paneId,
	);
	let outputStr = "";
