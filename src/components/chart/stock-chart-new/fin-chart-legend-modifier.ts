 ...
 */
export interface IFinanceLegendModifierOptions
...
export class FinChartLegendModifier extends ChartModifierBase2D {
	...
	private readonly xLineAnnotations: Map<string, LineAnnotation> = new Map<
		string,
		LineAnnotation
	>();
	private readonly yLineAnnotations: Map<string, LineAnnotation> = new Map<
		string,
		LineAnnotation
	>();
	private readonly paneModifiers: Map<SciChartSubSurface, CustomPaneModifier> =
		new Map<SciChartSubSurface, CustomPaneModifier>();
	...
	private readonly legendAnnotations: Map<string, FinChartLegendAnnotation> =
		new Map<string, FinChartLegendAnnotation>();
	...
	public constructor(options?: IFinanceLegendModifierOptions) {
		...
		this.legendOffsetXProperty =
			options?.legendOffsetX ?? this.legendOffsetXProperty;
		this.legendOffsetYProperty =
			options?.legendOffsetY ?? this.legendOffsetYProperty;
		...
		this.crosshairStrokeThickness =
			options?.crosshairStrokeThickness ?? this.crosshairStrokeThickness;
		this.crosshairStrokeDashArray =
			options?.crosshairStrokeDashArray ?? this.crosshairStrokeDashArray;
	...
	private getSciChartSurface(paneId: string) {
		return this.parentSurface.subCharts.find(
			(subChart) => subChart.id === paneId,
		);
	}
	...
	private updateMousePosition(scs: SciChartSubSurface) {
		...
		if (
			...
			if (!this.mousePoint) {
				...
				this.translatedMousePoint = translateFromCanvasToSeriesViewRect(
					this.mousePoint,
					scs.seriesViewRect,
				);
	...
	private update() {
		if (this.mousePosition === EMousePosition.SeriesArea) {
			...
			this.parentSurface.subCharts.forEach((scs) => {
				...
				xLineAnnotation.showLabel = scs.getXAxisById(
					xLineAnnotation.xAxisId,
				).drawLabels;
				...
				legendAnnotation.activeSciChartSurface = this
					.mousePositionSciChartSurface as SciChartSurface;
			});
		} else {
			this.parentSurface.subCharts.forEach((scs) => {
