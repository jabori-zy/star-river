...
import { Info, Minimize2, Plus, RefreshCw, Trash2 } from "lucide-react";
...
import type { IndicatorChartConfig, SeriesType } from "@/types/chart";
...
const TestPaneDeletion: React.FC = () => {
	...
	const handleDeleteBoth = (indicatorKeyStr: IndicatorKeyStr) => {
		const indicator = chartConfig.indicatorChartConfigs.find(
			...
			<Card>
				...
				<CardContent>
					<div className="space-y-2 mb-4">
						<p className="text-sm text-gray-600">测试不同的删除方式：</p>
					...
					<div className="mb-4 p-2 bg-blue-50 rounded-sm">
						...
						<div className="text-xs text-blue-700 space-y-1">
							...
							<div>
								已删除指标:{" "}
								{
									chartConfig.indicatorChartConfigs.filter((c) => c.isDelete)
										.length
								}{" "}
								个
							</div>
							{chartApiRef.current && (
								<div>
									当前 Pane 数量: {chartApiRef.current.panes().length} 个
								</div>
							)}
						</div>
					</div>
							...
							>
								<div className="flex items-center gap-2">
									<div
										...
										style={{
											backgroundColor:
												indicator.seriesConfigs[0]?.color || "#ccc",
										}}
									/>
									<span
										className={
											indicator.isDelete ? "line-through text-gray-400" : ""
										}
									>
										{indicator.seriesConfigs[0]?.name ||
											indicator.indicatorKeyStr}
										({indicator.isInMainChart ? "主图" : "子图"}) -
										{indicator.seriesConfigs[0]?.type || "unknown"}
									</span>
									<Badge
										variant={indicator.isInMainChart ? "default" : "secondary"}
										className="text-xs"
									>
										{indicator.indicatorKeyStr}
									</Badge>
								...
								<div className="flex gap-1">
									{!indicator.isDelete ? (
										<>
											{!indicator.isInMainChart && (
												<Button
													...
													title="只删除 Pane"
													onClick={() =>
														handleRemovePaneOnly(indicator.indicatorKeyStr)
													}
												>
											...
											<Button
												...
												title="删除配置"
												onClick={() =>
													handleDeleteIndicator(indicator.indicatorKeyStr)
												}
											>
											...
											<Button
												...
												title="同时删除"
												onClick={() =>
													handleDeleteBoth(indicator.indicatorKeyStr)
												}
											>
										...
										<Button
											...
											onClick={() =>
												handleRestoreIndicator(indicator.indicatorKeyStr)
											}
										>
											恢复
										</Button>
