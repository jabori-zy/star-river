...
export default function ColorPickerDemo() {
	...
	const [lastCompleteColor, setLastCompleteColor] =
		React.useState<ColorValue | null>(null);
		...
		<div className="container mx-auto p-6 space-y-8">
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>支持透明度调节的颜色选择器</CardDescription>
					</CardHeader>
					<CardContent className="space-y-4">
						<ColorPicker
							value={alphaColor}
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>使用自定义的预设颜色集合</CardDescription>
					</CardHeader>
					<CardContent className="space-y-4">
						<ColorPicker
							value={customColor}
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>常见品牌色彩的预设集合</CardDescription>
					</CardHeader>
					<CardContent className="space-y-4">
						<ColorPicker
							value={customColor}
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>演示颜色选择器的禁用状态</CardDescription>
				</CardHeader>
				<CardContent>
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						<ColorPicker value={disabledColor} disabled={true} />
						<div className="flex items-center gap-2">
							<div
					...
					<CardContent>
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							...
							<div className="space-y-2 text-sm">
								<div>
									<strong>HEX:</strong> <code>{lastCompleteColor.hex}</code>
								</div>
								<div>
									<strong>RGB:</strong>{" "}
									<code>
										rgb({lastCompleteColor.rgb.r}, {lastCompleteColor.rgb.g},{" "}
										{lastCompleteColor.rgb.b})
									</code>
								</div>
								<div>
									<strong>HSL:</strong>{" "}
									<code>
										hsl({lastCompleteColor.hsl.h}, {lastCompleteColor.hsl.s}%,{" "}
										{lastCompleteColor.hsl.l}%)
									</code>
								</div>
								<div>
									<strong>Alpha:</strong> <code>{lastCompleteColor.alpha}</code>
								</div>
							</div>
						</div>
					</CardContent>
