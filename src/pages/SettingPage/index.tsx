    ...
    return (
        <div className="flex flex-col gap-2">
            ...
            <Select 
                ...
                onValueChange={(value) => handleLocalizationChange(value as SupportLanguage)}
            >
                <SelectTrigger>
...
const SettingPage = () => {
    const { t } = useTranslation();
    const { systemConfig, isLoading, updateSystemConfigAction } = useSystemConfigStore();
    
    // 本地系统配置状态
    const [localSystemConfig, setLocalSystemConfig] = useState<SystemConfig | null>(null);
    // 系统设置是否发生变化
    const [systemConfigIsChanged, setSystemConfigIsChanged] = useState<boolean>(false);
    // 保存操作的加载状态
