...
function StrategyPageMainContent() {
  ...
  useEffect(() => {
    const tabsContent = (
        <Tabs defaultValue="flow" value={activeTab} onValueChange={setActiveTab} className="w-full max-w-md">
        <TabsList className="h-7 mt-1 p-0.5">
          <TabsTrigger value="flow" className="h-6 px-3 text-xs">策略节点</TabsTrigger>
          <TabsTrigger value="chart" className="h-6 px-3 text-xs">图表</TabsTrigger>
        </TabsList>
      </Tabs>
  ...
  return (
    <div className="h-screen flex flex-col bg-background overflow-hidden">
      <div className="flex-1 overflow-hidden relative" style={{ height: "calc(100vh - 40px)" }}>
        <Tabs value={activeTab} onValueChange={setActiveTab} className="h-full flex flex-col">
          <TabsContent value="flow" className="h-full flex-1 overflow-hidden mt-0 inset-0">
            {/* 高度改为窗口高度-40px */}
