...
export default function StrategyFlow({ strategy, onSaveStatusChange }: StrategyFlowProps) {
	...
	useEffect(() => {
		...
		// 如果没有节点和边，则创建一个开始节点
			...
			setTimeout(() => {
				...
				const startNodeData = createDefaultStartNodeData(strategy.id, strategy.name, t);
		...
		setNodes((currentNodes) => {
			...
			switch (dragNodeItem.nodeType) {
				case NodeType.KlineNode:
					defaultNodeData = createDefaultKlineNodeData(strategy.id, strategy.name, t);
					break;
				case NodeType.IndicatorNode:
					defaultNodeData = createDefaultIndicatorNodeData(strategy.id, strategy.name, t);
					break;
				case NodeType.IfElseNode:
					defaultNodeData = createDefaultIfElseNodeData(strategy.id, strategy.name, uniqueId, t);
					break;
				case NodeType.FuturesOrderNode:
					defaultNodeData = createDefaultFuturesOrderNodeData(strategy.id, strategy.name, t);
					break;
				case NodeType.PositionNode:
					defaultNodeData = createDefaultPositionNodeData(strategy.id, strategy.name, t);
					break;
				case NodeType.VariableNode:
					defaultNodeData = createDefaultVariableNodeData(strategy.id, strategy.name, t);
					break;
				default:
					defaultNodeData = {};
					break;
