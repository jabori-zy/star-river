		...
		if (!existingChart) {
			...
			setTimeout(() => {
				...
				const ma20Config: IndicatorChartConfig = {
					indicatorKeyStr:
						'symbol:BTCUSDT|interval:1m|indicator:MA|params:{"period":20}',
					isInMainChart: true,
					isDelete: false,
					seriesConfigs: [
						{
				...
				const ma50Config: IndicatorChartConfig = {
					indicatorKeyStr:
						'symbol:BTCUSDT|interval:1m|indicator:MA|params:{"period":50}',
					isInMainChart: true,
					isDelete: false,
					seriesConfigs: [
						{
				...
				const rsiConfig: IndicatorChartConfig = {
					indicatorKeyStr:
						'symbol:BTCUSDT|interval:1m|indicator:RSI|params:{"period":14}',
					isInMainChart: false,
					isDelete: false,
					seriesConfigs: [
						{
				...
				const macdConfig: IndicatorChartConfig = {
					indicatorKeyStr:
						'symbol:BTCUSDT|interval:1m|indicator:MACD|params:{"fastPeriod":12,"slowPeriod":26,"signalPeriod":9}',
					isInMainChart: false,
					isDelete: false,
					seriesConfigs: [
						{
				...
				const volumeConfig: IndicatorChartConfig = {
					indicatorKeyStr:
						"symbol:BTCUSDT|interval:1m|indicator:VOLUME|params:{}",
					isInMainChart: false,
					isDelete: false,
					seriesConfigs: [
						{
					...
					<div className="space-y-2">
						...
						<div className="flex gap-4 text-sm">
							...
							{currentChart && (
								<>
									<div>
										<span className="font-medium">主图指标:</span>{" "}
										{
											currentChart.indicatorChartConfigs.filter(
												(c) => c.isInMainChart && !c.isDelete,
											).length
										}
									</div>
									<div>
										<span className="font-medium">子图指标:</span>{" "}
										{
											currentChart.indicatorChartConfigs.filter(
												(c) => !c.isInMainChart && !c.isDelete,
											).length
