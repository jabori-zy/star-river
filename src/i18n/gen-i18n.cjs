...
function parseTranslationFile(filePath) {
    try {
        ...
        const match = content.match(/const\s+translation\s*=\s*({[\s\S]*?})\s*;?\s*export/);
        if (!match) {
...
function processLanguage(targetLang) {
    ...
    // 获取基础语言目录下的所有 .ts 文件
    const baseFiles = fs.readdirSync(baseLangDir).filter(file => file.endsWith('.ts'));
    ...
    baseFiles.forEach(fileName => {
        ...
        if (fs.existsSync(targetFilePath)) {
            ...
            console.log(`  Target file exists with ${existingKeyPaths.length} keys:`, existingKeyPaths);
        } else {
