...
interface BacktestOrderDataState {
  ...
  // 添加新订单数据到指定的组合键中
  addOrderData: (exchange: string, symbol: string, data: VirtualOrder[]) => void;
  
  // 添加单个订单到指定的组合键中
  addSingleOrder: (exchange: string, symbol: string, order: VirtualOrder) => void;
  ...
  // 获取指定订单
  getOrderById: (exchange: string, symbol: string, orderId: string) => VirtualOrder | undefined;

  // 获取最新订单
  getLatestOrder: (exchange: string, symbol: string) => VirtualOrder | undefined; 
...
export const useBacktestOrderDataStore = create<BacktestOrderDataState>((set, get) => ({
  // 初始化订单数据存储
  orderData: {},
  
  // 添加订单数据到指定组合键中
  addOrderData: (exchange, symbol, data) => set((state) => {
  ...
  // 添加单个订单到指定组合键中
  addSingleOrder: (exchange, symbol, order) => set((state) => {
    const orderKey = createOrderKey(exchange, symbol);
    const currentData = state.orderData[orderKey] || [];
  ...
  // 清空特定组合键的所有订单数据
  clearOrderData: (exchange, symbol) => set((state) => {
    const orderKey = createOrderKey(exchange, symbol);
    return {
      orderData: {
        ...state.orderData,
